...
| Tree engine under cache pressure | ≥1.5× point, ≥1.2× CI lower bound | Modal A10G run `ap-5jH2jxYxWXapTqAGcXjcKK`, after 96 unrelated long-prefix requests per arm: stock evicted the parent before the first child while tree pinning preserved all 2,406 parent tokens. Sibling-time VGE was **1.186×**, paired-bootstrap 95% CI **[0.994×, 1.530×]**. Explicit ownership helps under pressure, but still does not clear the target. | **FAIL** |
