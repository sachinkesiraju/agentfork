...
| Snapshot lifecycle + thread-safe reaper (checkpoint/restart/migration, artifact handoff, pdeathsig shim) | crashed VMM restarts with state; branch migrates to a fresh host; winner artifact extracted; reaper spawns fan out under threads | Recorded 2026-07-18 on Firecracker v1.16.1, aarch64 (Apple M3 Pro, Lima/nested KVM): **checkpoint → SIGKILL the VMM → restart** brought the branch back with its overlay marker intact; **export_bundle → import into a fresh sandbox (different work dir = simulated other host) → restart** migrated the branch with its overlay; **kill_losers(winner) → export_artifact** extracted the winner's `/tmp/workspace` tar (10 KiB) with the correct distinct output; the `pdeathsig="shim"` reaper armed `PR_SET_PDEATHSIG` via a re-exec launcher, spawned 16 branches concurrently, and an orphan died when its supervisor was SIGKILLed (signal survived `execve`). Linux suite 167/167 on the same host. | PASS |
...
through it between branches at `U = 17408` tokens per gap against a
break-even of `U* = C - P = 16384`. The tree is four levels deep (root context
-> 3 approach branches with their own committed reasoning -> 10 candidates ->
3 verification forks), so a leaf can reuse its whole lineage rather than just
the root. Recorded 2026-07-29 on the 2 vCPU CPU box (no GPU, no `/dev/kvm`;
`ReaperSandbox`, transformer forward stubbed):
...
| KV still pinned after killing 11 losers | 0 | 9,802 (= the winner's own committed prefix, exactly) |
...
the root prefix (asserted in `tests/test_race_demo.py`). The depth adds a
sharper boundary: `U* = C - P` only protects the *root*, so below it the stock
arm keeps the shared context on every candidate and still loses subtrees
(root hit `1.0`, full-lineage hit `0.33`) unless the pool also has room for
every sibling subtree at once. Pinning is `1.0` in every regime. Wall clock is secondary
