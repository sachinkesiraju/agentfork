...
python demo/fc_demo.py   # real-Firecracker demo (Linux + /dev/kvm; see --help)
...
2. **Sandbox fork**: `agentfork/sandbox/fc_bench.py` measures Firecracker
   ...
   can itself be forked again; `demo/fc_demo.py` runs the full fork/kill
   lifecycle against real microVMs. Guest networking, identity, and readiness
   are not handled.
